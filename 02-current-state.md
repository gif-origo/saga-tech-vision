...
- **SagaPlus (Angular + .NET backend)** — the newest user-facing functionality, built as Angular web modules backed by their own modern .NET backend services with business logic, data access, and domain rules. SagaPlus is not just a UI layer — it is a full-stack platform with its own API surface. Modules can be accessed directly in a browser by navigating to a URL, which works well for simpler, self-contained workflows. However, when end users need to work across multiple modules and systems simultaneously — switching between registration, journal, scheduling, and third-party tools — the browser-only experience becomes fragmented. This is a key motivation for the shell strategy ([Section 5.1.4](05-architecture-modernization.md#514-shell-strategy--wpf-modernization--standardized-module-contract)). In the current production setup, SagaPlus modules are also embedded inside the legacy Delphi desktop shell. The SagaPlus backend represents an opportunity to build a well-designed API platform for third-party integrations — see [Section 5.2.5](06-interoperability-ehds.md#525-sagaplus-as-api-platform).
- **Existing module portfolio** — beyond the four product pillars, Saga includes a broad range of specialized modules that serve specific clinical and administrative needs across the health system. These include MedCard (medication management), Antenatal (pregnancy care), Careteams (Meðvera — group care management), ADT (admission/discharge/transfer), Blazor-based web applications, reporting tools, and many others. These modules span all technology layers — some are Delphi-based, some are WPF, some are Blazor, and some are SagaPlus. They represent a significant portion of the value Saga delivers to healthcare organizations and will continue to be actively maintained and enhanced with new features. This vision does not propose replacing or deprecating these modules — they are part of the living system.
